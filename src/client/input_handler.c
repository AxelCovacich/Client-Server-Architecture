...
json_build_result build_json_from_input(char *raw_input) {
    ...
    if (strcmp(command, "unlock_client") == 0) {

        return build_json_for_unlock_client(command);
    }
...
json_build_result build_json_for_unlock_client(char *command) {
    ...
    char *client_to_unlock = strtok(NULL, " ");
    char *secret_phrase = strtok(NULL, " ");

    if (client_to_unlock == NULL || secret_phrase == NULL) {
        fprintf(stderr, "Error: unlock_client requires <client_to_unlock> and <secret_phrase>.\n"); // NOLINT
        return (json_build_result){.json_string = NULL, .status = JSON_BUILD_ERROR_SYNTAX};
    }
    ...
    if (root == NULL) {
        printf("Error creating the cJSON object...\n");
        logger_log("Input_handler", ERROR, "Error creating the cJSON object for unlock_client.");
        return (json_build_result){.json_string = NULL, .status = JSON_BUILD_ERROR_MEMORY};
    }
    ...
    cJSON *payload = cJSON_CreateObject();
    cJSON_AddStringToObject(payload, "client_to_unlock", client_to_unlock);
    cJSON_AddStringToObject(payload, "secret_phrase", secret_phrase);
    cJSON_AddItemToObject(root, "payload", payload);
    char *json_string = cJSON_PrintUnformatted(root);
    cJSON_Delete(root);
    return (json_build_result){.json_string = json_string, .status = JSON_BUILD_SUCCESS};
