...
|  # | Server location                                                                      | Client location                                         | `SERVER_ADDR` (what to give the client)                                                                |                                                           Ports to use (TCP / UDP) | Notes / requirements                                                                                                                                                         |
| -: | ------------------------------------------------------------------------------------ | ------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------: | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
|  1 | Server on **host** (local machine)                                                   | Client on **same host** (local)                         | `localhost` or `127.0.0.1`                                                                             |                                  `TCP_PORT=<server_port>` `UDP_PORT=<server_port>` | No Docker networking involved.                                                                                                                                               |
|  2 | Server on **host**                                                                   | Client **in Docker** on same host                       | `172.17.0.1` (docker0 gateway) **or** `host.docker.internal` (if available)                            |                                                      client uses same server ports | Client sees host at Docker bridge gateway. No `ports:` needed for client.                                                                                                    |
|  3 | Server **in Docker** on host (container)                                             | Client on **same host** (local)                         | `localhost` or `127.0.0.1`                                                                             | **Host port** (left side of mapping) — e.g. if compose has `8888:8888`, use `8888` | You **must** map container ports to host (e.g. `8888:8888`); open firewall if remote clients will connect.                                                                   |
|  4 | Server **in Docker** and Client **in Docker** within same `docker-compose`           | Client in the same compose/network                      | Docker service name e.g. `galileo-server`                                                              |         Use same port that the server listens on inside container (container port) | Docker's internal DNS resolves the service name; no host mapping required.                                                                                                   |
|  5 | Server **in Docker** on host A (remote)                                              | Client on **host B (another machine)**                  | Public IP or domain of host A (e.g. `203.0.113.5`)                                                     |                         **Host A port** (the host-exposed port) e.g. `9001`/`9000` | Ensure host A forwards/maps the container port and firewall/router allows inbound TCP/UDP.                                                                                   |
|  6 | Server on **host A** (remote)                                                        | Client **in Docker** on host B                          | Public IP or domain of host A                                                                          |                                                             Host A port (as above) | Same as #5 — client inside Docker will connect to remote host’s public IP and port.                                                                                          |
|  7 | Server **in Docker** on host A                                                       | Client **in Docker** on host B                          | Public IP or domain of host A                                                                          |                                                         Host A port (host-exposed) | Same as #5; both containers are on different hosts — use public/forwarded ports.                                                                                             |
|  8 | Server on **host** but client must be reachable for server-initiated new connections | Depends: if server initiates to client IP/port directly | If server initiates *new* connection to client, client must be reachable: use host IP + forwarded port |                   Client must expose port on its host (map container port to host) | Prefer design where client **initiates** first (keepalive) so NAT/NAT traversal is easier. If server must init, expose client port via `ports:` or use `network_mode: host`. |
|  9 | Quick dev/debug alternative (both on same host)                                      | Both inside Docker but you want host network            | `localhost`                                                                                            |                                               Use the ports your server listens on | Use `network_mode: "host"` (container shares host network). No port mapping needed, but less isolation.                                                                      |
...
4. **Create dashboards:**
   ...
   - Metrics will appear to add to the dashboard once the server detects them. Server must be running and events should already had happend in order to add the metrics.

### Important notes:
- The server must be running and exposing its metrics endpoint (default port: `8086`).
- In the folder "monitoring" there are two `prometheus.yml` files, one to run by default on docker, the other for local running outside docker.
